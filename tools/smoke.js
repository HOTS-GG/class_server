...
check('정답 미노출', !JSON.stringify(v1).includes('answerChoiceId'));
...
check('객관식 자동 채점 (25점)', row1?.score === 25);
...
check('서술형 수동 채점 반영 (총 33점)', graded.score === 33);
...
check('결과 CSV에 학생/점수 포함', csvText.includes('김민준') && csvText.includes('33'));

// ── 이벤트 로그(JSONL) ─────────────────────────────
check('답안 JSONL 기록', server.db.readEvents(`answers-${exam.id}`).length >= 6);
